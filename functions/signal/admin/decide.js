...
import { store, checkDecisionSig, issueToken, sendMail, loginMail, origin, page, esc } from "../_lib.js";
...
export async function onRequestGet({ request, env }) {
  ...
  return page("Signal 登録の承認", `
    ...
    <p class="muted" style="margin-top:20px">承認すると 15 分有効のログインリンクが本人に届きます。名簿(Mautic)への転記は別途。</p>`);
}
...
export async function onRequestPost({ request, env }) {
  ...
  if (action === "approve") {
    try {
      const tok = await issueToken(env, acct.email, x.dec.next);
      await sendMail(env, acct.email, loginMail(`${origin(env, request)}/signal/login/?t=${tok}`, acct.name));
