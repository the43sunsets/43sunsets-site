...
import { store, ingestOk, adminOk, json, normEmail, issueToken, sendMail, loginMail, origin, APPROVED_TOKEN_SECONDS } from "../_lib.js";
...
export async function onRequestGet({ request, env }) {
  ...
  const accounts = await kv.listAccounts();
...
export async function onRequestPost({ request, env }) {
  if (!ingestOk(request, env)) return json({ ok: false, error: "unauthorized" }, 401);
  const kv = store(env); if (!kv) return json({ ok: false, error: "no store" }, 503);
  ...
  const acct = await kv.getAccount(email); if (!acct) return json({ ok: false, error: "not found" }, 404);
