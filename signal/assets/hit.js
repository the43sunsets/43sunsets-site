...
// remembers the first utm_source it saw, and pings /signal/hit once per visit. Disable: localStorage.setItem('c43_optout','1').
(function(){try{
  ...
  var path=location.pathname+(location.pathname.indexOf('/signal/company/')===0?location.search:'');   // 9/5: company card views carry ?id=C-<n> (no personal data)
  var body=JSON.stringify({vid:vid,src:src,path:path});
  if(navigator.sendBeacon){navigator.sendBeacon('/signal/hit',new Blob([body],{type:'application/json'}));}
  else{fetch('/signal/hit',{method:'POST',headers:{'content-type':'application/json'},body:body,keepalive:true});}
}catch(e){}})();
